  ...
  if($num_activities->post_count > 0): ?>
    <div class="media<?php if(has_term('', 'priority_keywords')){ echo ' keyword'; } ?>">
      <div class="media-left">
        <span class="result-type-icon impact" data-toggle="tooltip" data-placement="top" title="<?php _e('Impact', 'usaidralf'); ?>"></span>
      </div>
                ...
                <div class="row">
                  <div class="col-sm-4 col-md-3">
                    <div class="sector-icon">
                      <a href="<?php echo $sector_url; ?>" class="parent-sector" data-toggle="tooltip" data-placement="top" title="<?php echo $sector_name; ?>">
                        <img src="<?php the_field('sector_icon', $acf_sector_id); ?>" class="img-circle img-responsive" alt="<?php echo $sector_name; ?> <?php _e('Sector', 'usaidralf'); ?>" style="background-color:<?php the_field('sector_color', $acf_sector_id); ?>;" />
                      </a>
                      <?php if($sector->parent > 0): ?>
                        <a href="<?php echo esc_url(get_term_link($sector->term_id, 'sectors')); ?>" class="sub-sector" data-toggle="tooltip" data-placement="top" title="<?php echo $sector->name; ?>">
                          <img src="<?php the_field('sector_icon', $acf_sub_sector_id); ?>" class="img-circle img-responsive" alt="<?php echo $sector->name; ?> <?php _e('Sector', 'usaidralf'); ?>" style="background-color:<?php the_field('sector_color', $acf_sub_sector_id); ?>" />
                        </a>
                      <?php endif; ?>
                    </div>
                  </div>
                  <div class="col-sm-8 col-md-9">
                    <div class="number-activities">
                      <a href="<?php the_permalink(); ?>"><span><?php echo $num_activities->post_count; ?></span><?php _e('Activities', 'usaidralf'); ?></a>
                    </div>
                  </div>
                </div>
          
        <?php endforeach; else: ?>
         <p><?php esc_html_e('No impacts are associated with this activity.', 'usaidralf'); ?></p>
        <?php endif; ?>
      </div>
    </div>
<?php else: 
  //if(is_page('quick-select-results')){
    if(isset($_POST['factor']) && !empty($_POST['factor'])){
      $impact_title = '<span style="text-transform:capitalize;">' . get_the_title() . '</span>';
      echo '<div class="media-body" style="padding-top:40px;"><p>' . printf(esc_html__('No Activities have been associated with the %s impact.', 'usaidralf'), $impact_title) . '</p></div>';
    }
    else{
